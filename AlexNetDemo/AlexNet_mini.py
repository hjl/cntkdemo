...
# run AlexNet-style training on some other image sets
...
#log_dir = None
INPUT_IMAGE_DIRECTORY = r'E:\local\TestData\training-images'
MODEL_DIRECTORY = r'E:\local\TestData\training-images\Models'
LOG_DIRECTORY = r'E:\local\TestData\training-images\Logs'

abs_path = INPUT_IMAGE_DIRECTORY
...
def create_trainer(network, epoch_size, num_quantization_bits, printer, block_size, warm_up):
    ...
    # Create trainer
    if block_size != None:
        parameter_learner = block_momentum_learner(local_learner, block_size=block_size)
    else:
...
def alexnet_train_and_eval(train_data, test_data, num_quantization_bits=32, block_size=4, warm_up=0, minibatch_size=2, epoch_size = 8, max_epochs=112,
...
# TODO - what is _cntk_py vs cntk_py?
...
if __name__=='__main__':
    ...
    data_path = os.path.join(abs_path, "hjl")
    # TODO - fix up path and put useable test image set somewhere
...
# TODO - mostly don't need argparse for now, but fix if we want this
    ...
    print('train_data at', train_data)
    print('test_data at', test_data)
